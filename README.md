...
即 `虚拟地址 >= p->s` 时中止该进程。如果没有实现跳过这个特性，并且实现页的映射，很可能遇到`freewalk: leaf`的panic，因为释放页表项，是按照sz释放的，过高的分配就会导致一些pte没有卸载，于是回收页表目录的内存页失败。
...
做法的缺点是，`vmprint`的测试会失败，因为目录页的标志位会发生变化

### 处理系统调用read/write/pipe的缺页

`usertrap()`只会处理用户态代码产生的缺页，也就是MMU翻译时，由CPU触发的异常trap，做好准备工作，跳转到`usertrap`。而系统调用`write`等，发生在内核态，是直接使用`vm.c:copyin -> walkaddr()`，用代码模拟MMU的行为，查找用户进程的虚拟地址的物理地址，以便向内核态拷贝数据，如果此前虚拟地址并没映射，就会直接返回-1，而不会转向`usertrap`。因此，可以在专门为用户态查找的`walkaddr`中模拟`usertrap()`中的行为。

要注意的是，在`walkaddr`中，只有在确定页表未映射时，才应该进入va是否sz越界，是否进入guard的检查。
